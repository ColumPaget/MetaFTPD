...
else
{
	...
	if (StrLen(p_Addr)) 
	{
		...
		//very naughty, but it works. We used '\0' to shorten the string
		//now we put the '%' character back!
		if (ptr) *ptr='%'; 
	}
...
sa6=(struct sockaddr_in6 *) &sa;
if_indextoname(sa6->sin6_scope_id, Tempstr);

*Addr=MCatStr(*Addr,"%",Tempstr,NULL);
}
...
if (result==0)
{
	...
	*LocalPort=atoi(Tempstr);
	...
	if ((sa.ss_family==AF_INET6) && (! strrchr(*LocalAddress,'%')))
	{
		Tempstr=SetStrLen(Tempstr,255);
		sa6=(struct sockaddr_in6 *) &sa;
		if_indextoname(sa6->sin6_scope_id, Tempstr);

		*LocalAddress=MCatStr(*LocalAddress,"%",Tempstr,NULL);
	}
